...
function hideModal(modalId) {
    ...
    if (form) {
        form.reset();
        // Eliminar atributos de dataset para asegurar que no queden datos anteriores
    ...
    tableBody.innerHTML = vehicles.data.map(vehicle => `
        <tr>
            ...
            <td>
                ...
                <button class="btn btn-success btn-sold" data-id="${vehicle.id}" ${vehicle.vendido ? 'disabled' : ''}>Marcar Vendido</button>
...
async function loadModalSelects() {
    try {
        ...
        const vehicleOptions = vehicles.data
            .filter(vehicle => !vehicle.vendido) // Solo mostrar vehículos no vendidos en selects
            .map(vehicle => 
    ...
    if (vehicleForm) {
        vehicleForm.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                await loadDashboardData(); // Recargar datos para actualizar la tabla
            } catch (error) {
...
function fillUserForm(userData) {
    ...
    form.role.value = userData.rol || userData.role || 'user';
...
function fillPurchaseForm(purchaseData) {
    ...
    if (form.paymentMethod) {
        form.paymentMethod.value = purchaseData.metodo_pago || purchaseData.paymentMethod || 'Tarjeta';
    }
    
    if (form.status) {
        form.status.value = purchaseData.estado || purchaseData.status || 'Completado';
    }
...
function handleDelete(type, id) {
    ...
    confirmYesBtn.onclick = async () => {
        try {
            ...
            await loadDashboardData(); // Asegurarnos de recargar los datos después de eliminar
        } catch (error) {
