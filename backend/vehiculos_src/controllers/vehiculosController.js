...
const postVehiculo = async (req, res) => {
    let { anio, kilometraje, marca, modelo, precio } = req.body;
    ...
    console.log("Campos limpios:", { marca, modelo, anio, precio, kilometraje }); // Verifica los valores limpios

    // Validación: Asegurarse de que todos los campos están presentes
    if (!marca || !modelo || !anio || !precio || !kilometraje) {
...
const putVehiculo = async (req, res) => {
    const { id } = req.params;
    const { marca, modelo, anio, precio, kilometraje } = req.body;
    try {
        const result = await Vehiculo.actualizarVehiculo(id, { marca, modelo, anio, precio, kilometraje });
...
const buscarVehiculos = async (req, res) => {
    try {
        const { anio, kilometraje, marca, modelo, precio_max, precio_min } = req.body;
...
const getYearsPorMarcaYModelo = async (req, res) => {
    const { marca, modelo } = req.params;
    try {
        const anios = await Vehiculo.obtenerYearsPorMarcaYModelo(marca, modelo);
        res.status(200).json({
            success: true,
            count: anios.length,
            data: anios
