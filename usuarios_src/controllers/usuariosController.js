...
router.put("/api/usuarios/password-change", async (req, res) => {
    ...
    try {
        // Como no estamos usando bcrypt, guardamos la contraseña en texto plano
        // En un entorno de producción, deberías usar bcrypt u otra biblioteca de hash
        const result = await Usuario.actualizarUsuario(id, null, null, null, null, nuevaContrasena);
        if (result.affectedRows > 0) {
